...
DEFAULT_DROPLET_NAME="op-energy-dev"
...
if [ ! -d ".state/$DROPLET_IP" ]; then

    mkdir -p ".state/$DROPLET_IP"
    echo "fetching configuration from droplet to use them later with morph"
    scp "root@$DROPLET_IP":/etc/nixos/networking.nix ".state/$DROPLET_IP/networking.nix"
    scp "root@$DROPLET_IP":/etc/nixos/host.nix ".state/$DROPLET_IP/host.nix"
    scp "root@$DROPLET_IP":/etc/nixos/hardware-configuration.nix ".state/$DROPLET_IP/hardware-configuration.nix"
    scp "root@$DROPLET_IP":/etc/nixos/configuration.nix ".state/$DROPLET_IP/configuration.nix"
fi

# generate unique secrets for instance
echo "generating DB secrets"
if [ ! -d "./.state/$DROPLET_IP/private" ]; then
    ../gen-psk.sh ".state/$DROPLET_IP/private" signet
else
    echo "INFO: secrets already exists"
fi

echo "deploying configuration to a NixOS instance"
